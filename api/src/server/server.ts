...
export async function server(config: ServerConfig) {
  ...
  const kinetic = new Kinetic(
    config,
    sdk,
    Keypair.fromMnemonic(config.airdropMnemonic)
  );
  ...
  app.listen(Number(config.port), '0.0.0.0').on('listening', async () => {
    ...
    console.log(
      `⬢ Kinetic: Connected to App: ${sdk.config?.app.name} ${sdk.config?.app.index} `
    );
    console.log(
      `⬢ Kinetic: Connected to API: ${sdk.config?.api.name} ${sdk.config?.api.version} `
    );
    console.log(
    ...
    sdk.config?.mints.forEach((mint) => {
      console.log(
        `⬢ Kinetic: Mint: ${mint.name} ${mint.publicKey} (${
          mint.decimals
        } decimals) (Airdrop: ${
